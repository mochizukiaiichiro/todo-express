...
describe('todoDbService', () => {
    ...
    test('insertで追加した複数のTodoの取得できる。', () => {
      ...
      expect(todos).toEqual(
        expect.arrayContaining(
          expectedTodos.map((t) => expect.objectContaining({ ...t, completed: false }))
        )
      );
    });
  });
  ...
  describe('delete (削除機能)', () => {
    ...
    test('複数Todoのうち指定IDのみ削除される', () => {
      ...
      // --- 検証 ---
      expect(dbs.getAll()).toEqual([expect.objectContaining({ id: id2, todoName: 'test2' })]);
    });
  ...
  describe('getCompleted（指定した completed 状態の Todo を取得）', () => {
    ...
    test('completed=false の Todo のみ取得できる', () => {
      ...
      expect(todos).toEqual([expect.objectContaining({ id, todoName, completed: false })]);
    });
    ...
    test('completed=true の Todo のみ取得できる', () => {
      ...
      expect(todos).toEqual([expect.objectContaining({ id, todoName, completed: true })]);
    });
