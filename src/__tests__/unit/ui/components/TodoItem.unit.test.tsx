...
describe('TodoItem', () => {
  ...
  test('todoName が表示される', () => {
    // --- 実行 ---
    render(<TodoItem todo={todo} onChangeCheckbox={jest.fn()} onClickDeleteButton={jest.fn()} />);
  ...
  test('チェックボックス変更でonChangeCheckboxが呼ばれる', async () => {
    ...
    // --- 実行 ---
    render(
      <TodoItem todo={todo} onChangeCheckbox={onChangeCheckbox} onClickDeleteButton={jest.fn()} />
    );
